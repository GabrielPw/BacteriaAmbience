package Simulator.Graphics.Utils;
...
public class Shader {
    ...
    public void addUniform3fv(String uniformName, Vector3f vector) {
        int uniformLocation = GL20.glGetUniformLocation(ID, uniformName);
        if (uniformLocation != -1) {
            ...
            GL20.glUniform3fv(uniformLocation, buffer);
        } else {
            System.err.println("Uniform " + uniformName + " not found in shader program.");
        }
    }
    ...
    private static int loadShader(String file, int type) {
        ...
        try {
            InputStream inputStream = Shader.class.getResourceAsStream("/shaders/" + file);
            if (inputStream == null) {
                System.err.println("Simulator.Game.Shader file not found: " + file);
                System.exit(-1);
            }
